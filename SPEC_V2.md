...
```
strength < 1.5 (약한 시그널):
  post-only limit 5회 추격 (maker 0.02%)
  미체결 → 진입 포기 (taker 없음)

strength ≥ 1.5 (강한 시그널):
  post-only 3회 추격 (maker 시도)
  미체결 → limit 주문 (taker 0.05% 허용)
  이유: 강한 모멘텀은 체결이 더 중요

SL/TP 알고:
  SL: limit-on-trigger (maker) + sl_failsafe 백업
...
러너:
  trail_distance = max(ATR × 1.5, 수익의 25%)
  시간 감쇠: 30분 이후 trail 30% 축소
