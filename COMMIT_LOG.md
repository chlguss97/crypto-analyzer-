...
> Updated: 2026-05-22 21:58:17    
> Total commits: 431 (2026-04-03 → 2026-05-22)

Claude 가 질문/변경 작업 시 이 파일을 참고해서 과거 변경 이력 컨텍스트를 확보합니다. 수동 편집 금지 — 다음 커밋 시 덮어써집니다.

## 2026-05-22
- `439cd110` fix: AUDIT v4 최종 — HIGH 2건 + MEDIUM 4건 수정
- `51e1545b` fix: AUDIT HIGH 4건 수정
- `d5e3b8a7` feat: SPEC v4 — Minimal DGT Grid (전면 리팩터)
- `edaf0e8b` refactor: 하드코딩 수치 → config 이관 + 수수료 버그 수정
- `9a43277f` fix: CUSUM 과민 보정 — 감쇄 0.95→0.85, threshold 2.0→3.0
...
- `ea379e55` fix: AUDIT 전수검사 결과 수정 — CRITICAL/HIGH 전부 처리
...
- `753e9301` fix: VPIN Extreme 0.7→0.85 (BTC 보정) + 단계 경계 조정
- `31161274` docs: 최신화 — Shadow WR 84.3%, z-score 임계값, VPIN 수정 반영
- `571b4311` fix: VPIN 볼륨 기반→건수 기반 버킷 (200건/버킷)
- `b9e41240` fix: VPIN 초기 버킷 5BTC→0.5BTC — 0.9+ 오탐 수정
- `fd4f15b1` debug: 게이트별 차단 통계 로그 추가 (30초마다)
- `a81c381a` fix: 모든 임계값 z-score/상대값 전환 — 절대값 0건
- `20489529` fix: Burst 임계값 절대값→z-score 전환 (프로 동일)
- `b4d387de` fix: 봇 시작 시 5분봉 캐시 초기화 — Hurst/Parkinson 즉시 계산
- `e30cae22` fix: 전수검사 5건 수정 — test삭제/ML피처누락/텔레그램버그
