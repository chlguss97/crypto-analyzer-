...
> Updated: 2026-04-29 15:27:21    
> Total commits: 282 (2026-04-03 → 2026-04-29)

Claude 가 질문/변경 작업 시 이 파일을 참고해서 과거 변경 이력 컨텍스트를 확보합니다. 수동 편집 금지 — 다음 커밋 시 덮어써집니다.

## 2026-04-29
- `e2bca74` config: 로그 시간대 KST (Asia/Seoul) — 내부 로직은 UTC 유지
...
- `bc4fe36` feat: Phase A 무조건 Go — 데이터 수집 가속 (마진 40% 유지)
- `c738694` feat: 약한 후보(weak_momentum) shadow 전용 — ML 데이터 2~3배 가속
- `7374430` chore: 디버그 로그 제거 + 운영 로그 유지
- `8891c6f` debug: _evaluate 조기리턴 로깅 추가
- `90aa805` debug: OKX 캔들 WS 디버그 로깅 추가 (임시)
...
- `bbc55b6` fix: CRITICAL — Binance combined stream URL 형식 수정
- `8b5f210` fix: Binance futures WS 복원 + 디버그 로깅 강화
- `19b9c87` fix: Binance futures 403 차단 → spot 엔드포인트 전환
- `7ff360b` fix: ml_engine asyncio.get_event_loop→get_running_loop (Python 3.12+ 호환)
- `fcd8924` chore: 모든 패키지 최신 버전으로 (버전 제한 제거)
- `1b2430f` chore: requirements.txt 정리 — 미사용 제거 + 최신 호환
- `f299558` fix: CRITICAL — websockets v16 호환 (async with → await + recv 루프)
...
- `3425061` fix: CRITICAL — Binance WS combined stream 파싱 누락
